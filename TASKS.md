...
| [x] | S11-T4 | **Surface guard state and reasons.** Render the same canonical event in CLI/TUI and panel: level, proposed action, evidence quality, reason and update time. Observe-only must be labelled; unknown must not render as healthy. | M | S11-T3 | — |
| [x] | S11-T5 | **Enforce opt-in boundary actions.** Default remains observe-only. A constrained run may suppress future retry/escalation/new work at task boundaries; hard stop may request cancellation only when the adapter declares and proves it. Never restart completed work or silently change flow mode. | M | S11-T3, S11-T4 | — |
...
| ✓ | ID | Task | Size | Depends on | Parallel? |
|---|---|---|---|---|---|
| [x] | S12-T1 | **Reject transient scope in `FsMemoryStore` (closes S9-T8).** A session-scoped write must fail before disk; project/user behaviour stays unchanged. Tick S9-T8 with this task. | S | — | ‖ S11-T2…T5 |
| [x] | S12-T2 | **Add daemon-backed durable memory.** Store project memory by canonical repository identity and user memory privately; keep session memory transient/fork-safe. Provenance, review state, expiry and visibility must round-trip without guessed defaults. | M | S11-T1, S12-T1 | — |
| [x] | S12-T3 | **Inject approved memory into real runs (closes S9-T7).** Use the existing token-budget selector in Single and Team prompt assembly. Pending/rejected/expired entries never reach a provider; unavailable memory degrades to no injection. Tick S9-T7 with this task after S12-T2 lands. | M | S12-T2 | — |
| [x] | S12-T4 | **Expose proposal review end to end.** List/accept/reject through the daemon and one user surface. Accept still requires explicit provenance; duplicate/concurrent review has deterministic first-result semantics and audit data. | M | S12-T2 | ‖ S12-T3 |
| [x] | S12-T5 | **Add immutable run-context entries.** Persist attributable facts, decisions, blockers, questions and artifact metadata; derive the current blackboard snapshot instead of overwriting a shared file. Forks do not inherit post-fork context. | M | S12-T2 | — |
| [x] | S12-T6 | **Hand artifacts to dependent tasks.** Resolve bounded artifact references into later prompts with producer provenance and context-budget accounting. Missing artifacts become named blockers; never scrape another worker's worktree implicitly. | M | S12-T3, S12-T5 | — |
| [x] | S12-T7 | **Show memory, context and artifacts.** CLI and panel read daemon records and show scope, provenance, review state and source task; list routes do not load large artifact bodies by default. | M | S12-T4, S12-T5 | ‖ S12-T6 |
...
| ✓ | ID | Task | Size | Depends on | Parallel? |
|---|---|---|---|---|---|
| [x] | S13-T1 | **Define task-scoped messages.** Persist unique, redacted, capped envelopes addressed to task/lead/orchestrator with conversation/reply ids, handled state and hop count. Durable provider personas and broadcast are out of scope. | M | S12-T5 | — |
| [x] | S13-T2 | **Route messages at task boundaries.** Drain validated requests after a task settles and make responses available to a later start/resume. Only request-artifact/blocker require replies; message/hop budgets end ping-pong with a named escalation. | M | S12-T6, S13-T1 | — |
